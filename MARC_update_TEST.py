...
def update_item_enum_chron(marc_input, marc_output_barcodes, marc_output_hldgs_state):
    ...
    #Make log file
    logger = logging.getLogger(__name__)
    logging.basicConfig(filename=('logs//' + marc_input + 'log.txt'), encoding='utf-8', level=logging.DEBUG)
    ...
    with open(marc_input, 'rb') as fh:
        ...
        for bib_record in reader:
            ...
            if ('876' or '877' or '878') in bib_record:
                ...
                # for each_stub in barcode_stubs:
                #     print(each_stub.as_marc())

                with open(marc_output_file, 'wb') as marc_file:
                    writer = MARCWriter(marc_file)

                    for each_stub in all_new_item_records:
                        writer.write(each_stub)
...
def item_update_dictionary(record, marc_output_file):
    ...
    # with open(marc_output_file, 'wb') as marc_file:
    #     for item_marc_record in all_new_item_records:
    #         marc_file.write(item_marc_record.as_marc())

    # print(len(all_new_item_records))
